...
\begin{equation}
    L_{E(x,y)} + L_x L_y = 2 \, \vy \otimes \vx - \vx \otimes \vy - g(\vx,\vy) I. \label{eq:Exy_xy}
\end{equation}
...
\begin{align}
    ...
    L_x L_y L_x & = - L_{E(x,y)} L_x  - \vx \otimes E(\vy,\vx) - g(\vx,\vy) L_x \label{eq:first}
\end{align}
since 
\[
...
\begin{align}
    ...
    & = L_{E(E(x,y),x)} -  \vx \otimes E(\vx,\vy) + E(\vx,\vy)\otimes \vx - g(\vx,\vy) L_x. \label{eq:second}
\end{align}
Next 
\[
...
\begin{align}
    ...
    & = g(\vx,\vx) L_y - 2  g(\vx,\vy) L_x - \vx \otimes E(\vx,\vy) + E(\vx,\vy)\otimes \vx . \label{eq:third}
\end{align}
...
If $f=I$ then 
\[
S(I)=\sum_{i=1}^d L_{e_i}^2 = \sum_{i=1}^d L_{e_i \otimes e_i} - \sum_{i=1}^d I = \sum_{i=1}^d L_{e_i \otimes e_i} -d I.
\]
Since 
\[
\sum_{i=1}^d L_{e_i \otimes e_i} (\vx)= \sum_{i=1}^d g(\ve_i,\vx) \ve_i = \vx ,
\]
\begin{equation}
S(I) = (1-d)I.  \label{eq:SI}
\end{equation}

If $f=L_{x \otimes y}$ then using (\ref{eq:xcrossytensorz}) and (\ref{eq:xtensorycrossz})
\begin{align*}
    S(L_{x \otimes y}) & = \sum_{i=1}^d L_{e_i} \circ L_{x \otimes y}  \circ L_{e_i} \\
    & = \sum_{i=1}^d L_{e_i} \circ L_{x \otimes E(y,e_i)} \\
    & = \sum_{i=1}^d L_{E(e_i,x)\otimes E(y,e_i)}.
\end{align*}
Using (\ref{eq:cyclic})
\begin{align*}
L_{E(e_i,x)\otimes E(y,e_i)} (\vz) & = g(E(\vy,\ve_i),\vz) E(\ve_i,\vx) \\
...
\begin{align*}
    S(L_{x \otimes y}) & = \sum_{i=1}^d E\qty(\vx,g(\ve_i,E(\vy,\vz)) \ve_i) \\
    & = E\qty(\vx,\sum_{i=1}^d g(\ve_i,E(\vy,\vz)) \ve_i) \\
    & = E(\vx,E(\vy,\vz)) = L_x L_y (\vz)
\end{align*}
and so
\begin{equation}
    S(L_{x \otimes y}) = L_x L_y \label{eq:Sxoy}.
\end{equation}

If $f=L_{y}$ then using (\ref{eq:third})
\begin{align}
    S(L_y) & = \sum_{i=1}^d L_{e_i} \circ L_{y} \circ L(e_i) \\
    & = \sum_{i=1}^d \qty(g(\ve_i,\ve_i) L_y - 2 g(\ve_i,\vy) L_{e_i} - \ve_i \otimes E(\ve_i,\vy) + E(\ve_i,\vy) \otimes \ve_i ).
\end{align}
...
For the third term 
\begin{align*}
\sum_{i=1}^d \ve_i \otimes E(\ve_i,\vy) (\vz) & = \sum_{i=1}^d g(\vz,E(\ve_i,\vy)) \ve_i = \sum_{i=1}^d g(\ve_i,E(\vy,\vz)) \ve_i \\
& = E(\vy,\vz) = L_y (\vz).
\end{align*}
In a similar way the fourth term is 
\begin{align*}
    \sum_{i=1}^d  E(\ve_i,\vy) \otimes \ve_i (\vz) & = \sum_{i=1}^d  E(\ve_i,\vy) g(\vz,\ve_i) \\
    & = E\qty(\sum_{i=1}^d g(\vz,\ve_i) \ve_i, \vy) = E(\vz,\vy) = - E(\vy,\vz) = - L_y (\vz).
\end{align*}
Hence we conclude that 
\begin{equation}
    S(L_y) = (d-4)L_y  \label{eq:Sy}.
\end{equation}

If $f=L_x L_y$ then 
\begin{align}
    S(L_xL_y) & = - S(L_{E(x,y)}) + 2 S(L_{y \otimes x}) - S(L_{x \otimes y}) - g(\vx,\vy) S(I)  & \text{using (\ref{eq:Exy_xy})} \nonumber \\
    & = -(d-4)L_{E(x,y)} + 2 S(L_{y \otimes x}) - S(L_{x \otimes y}) - g(\vx,\vy) S(I)  & \text{using (\ref{eq:Sy})} \nonumber \\
    & = -(d-4)L_{E(x,y)} + 2 L_y L_x - Lx Ly - g(\vx,\vy) S(I)  & \text{using (\ref{eq:Sxoy})} \nonumber \\
    & = -(d-4)L_{E(x,y)} + 2 L_y L_x - L_x L_y - (1-d) g(\vx,\vy) I  & \text{using (\ref{eq:SI})} . \label{eq:Sxy} 
\end{align}

Using this derivation we can compute the following transformation: 
\[
...
\[
g = \sum_{i=1}^d  L_{e_i} \circ L_x \circ (d-4) L_{e_i} = (d-4) S(L_x) = (d-4)^2 L_x.
\]
...
\]
For $L_{e_j} L_x $ we have 
\[
2 \sum_{j=1}^d  L_{e_j} L_x \circ L_{e_j} = 2 S(L_x) = 2 (d-4) L_x 
\]
while for $L_x L_{e_j}$ we have 
\[
L_x \sum_{j=1}^d L_{e_j} \circ L_{e_j} = L_x S(I) = (1-d)L_x.
\]
