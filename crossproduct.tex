...
\]
In this vector space the \emph{volume form} is (0,3) antisymmetric tensor
\[
...
where $N(\sigma)$ is the number of single transpositions that generate $\sigma$. Given a volume form, for $\phi \in \End(\cV)$
\[
\det(\phi) = \frac{\omega(\phi \ve_1, \phi \ve_2, \phi \ve_3)}{\omega( \ve_1, \ve_2,\ve_3)}
\]
where $\{ \ve_i \}_{i=1}^3$ is an orthonormal basis (we use an orthonormal basis to simplify the derivation
with the standard property $\ve_1=E(\ve_2,\ve_3)$, $\ve_2=E(\ve_3,\ve_1)$, $\ve_3=E(\ve_1,\ve_2)$). 
Define the volume form: 
\[
\omega(\vx_1,\vx_2,\vx_3)= g(\vx_1,E(\vx_2,\vx_3));
\]
We can use (\ref{eq:cyclic}) to show that the definition is antisymmetric. For example, 
\begin{align*}
\omega(\vx_3,\vx_2,\vx_1) & =g(\vx_3,E(\vx_2,\vx_1))=-g(\vx_3,E(\vx_1,\vx_2)) \\
& =-g(\vx_1,E(\vx_2,\vx_3))=-\omega(\vx_1,\vx_2,\vx_3)
\end{align*}
with $123 \to 321$ requiring a single transposition. Combine this with the definition of the determinant:
\begin{align*}
\det(\phi) & = \frac{g(\phi \ve_1, E(\phi \ve_2, \phi \ve_3) )}{g( \ve_1, E(\ve_2,\ve_3))} 
 = g(\phi \ve_1, E(\phi \ve_2, \phi \ve_3) ).
\end{align*}
